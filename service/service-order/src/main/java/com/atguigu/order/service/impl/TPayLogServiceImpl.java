...
public class TPayLogServiceImpl extends ServiceImpl<TPayLogMapper, TPayLog> implements TPayLogService {
    ...
    public Map createnative(String orderNo) {
        ...
        try {
            ...
            if(order==null){
                throw new GuliException(20001,"没有该订单信息");
            }
            Map m = new HashMap();
            //1、设置支付参数
            m.put("appid", "wx74862e0dfcf69954");
            m.put("mch_id", "1558950191");
            m.put("nonce_str", WXPayUtil.generateNonceStr());//这是生成随机的字符串
            m.put("body", order.getCourseTitle());//二维码名称
            m.put("out_trade_no", orderNo);//二维码标识
            m.put("total_fee", order.getTotalFee().multiply(new BigDecimal("100")).longValue()+"");
            m.put("spbill_create_ip", "127.0.0.1");
            m.put("notify_url", "http://guli.shop/api/order/weixinPay/weixinNotify\n");//回调地址
            m.put("trade_type", "NATIVE");//支付类型

            //2、HTTPClient来根据URL访问第三方接口并且传递参数 地址是微信提供的固定地址 写好的工具类
